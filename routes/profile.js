...
router.put('/updateWithId/:id', function(request, response) {
    ...
    bikeRoute.model.findByIdAndUpdate(id, routeObj, {
        upsert: true
    }, function(error, userRoute) {
        if (error) {
...
router.put('/keywordsearch', function(request, response) {
    ...
    bikeRoute.model.find({
        $or: [{
            'startLocation': vm.start
        }, {
            'endLocation': vm.end
        }, {
            'startLocation': vm.end
        }, {
            'endLocation': vm.start
        }]
    }, function(error, bikeRoute) {
        if (error) {
